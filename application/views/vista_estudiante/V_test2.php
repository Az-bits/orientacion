...
<body>
  ...
  <form class="form-register" action="<?php echo base_url(Hasher::make(8))?>" method="POST">
  <!-- <form class="form-register" id="guardar_seguimineto_eva" method="POST"> -->
    <div style="text-align: center">
      <h3>TEST DE INTELIGENCIA MULTIPLES -HOWARD GARDNER-</h3>
    </div>
    <h5><br> I) Lee cada una de las afirmaciones.<br>II) Si expresan características fuertes en tu persona y te parece que la afirmación es veraz entonces seleccione "VERDAD" y si no lo es, seleccione "FALSO".<br> III) Responde a todas las preguntas sin omitir ninguna.</h5>
    <table border="1" id="" class="" style="border-color:cyan;border-spacing:0px; width:100%"><br>
      ...
      <tbody> 
      <?php $con=1; foreach ($listar as $value) { ?>
            <tr>

              <input type="hidden" name="idest" value="<?php echo $idest; ?>">
              <input type="hidden" name="idpreg[]" value="<?php echo $value->idpreguntas2; ?>">
              <td ><?php echo $value->idpreguntas2; ?></td>
              <td><?php echo $value->nom_pregunta2; ?></td>
              <td>
                <label><input  style="height:15px; width:15px" type="radio" name="respuesta<?php echo $value->idpreguntas2; ?>" required value="0"> FALSO</label><br>
                <label><input  style="height:15px; width:15px" type="radio" name="respuesta<?php echo $value->idpreguntas2; ?>" required value="1">VERDAD</label>
              </td>
            </tr>
        <?php   } ?>
      </tbody>
