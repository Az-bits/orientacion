...
<body>
  <div class=container-fluid>
    <form class="form-register" method="POST" action="<?php echo base_url(Hasher::make(277)) ?>">
    <div style="text-align: center">
        <h4>PARA REALIZAR EL TEST DE INTELIGENCIA INTRODUZCA LOS DATOS CORRESPONDIENTES</h4>
      </div>
      ...
      <select id="txtColegio" name="txtColegio" class="controls">
        <option value="0">Colegio</option>
      </select>

      <input class="botons" type="submit" value="REALIZAR TEST DE INTELIGENCIA">
...
<script src="http://cceorientacionvocacional.upea.bo/assets/admin/assets/js/global-plugins.js"></script>
    ...
    if (ci.length > 3) {
      $.post('<?php echo base_url(); ?>Controller_estudiante/C_buscar_estudiante', {
        ...
        if (datos) {
          ...
          $("#txtNombres").val(datos.nombre_est).attr('disabled', 'disabled');
          $("#txtApellidos").val(datos.apellido_est).attr('disabled', 'disabled');
          $("#txtCelular").val(datos.celular_est).attr('disabled', 'disabled');
